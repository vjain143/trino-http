...
{
    private final Split split = new Split("http://127.0.0.1/test.file");

    @Test
    public void testAddresses()
    {
        // http split with default port
        Split httpSplit = new Split("http://example.com/example");
        ...
        httpSplit = new Split("http://example.com:8080/example");
        assertEquals(httpSplit.getAddresses(), ImmutableList.of(HostAddress.fromParts("example.com", 8080)));
        assertEquals(httpSplit.isRemotelyAccessible(), true);

        // http split with default port
        Split httpsSplit = new Split("https://example.com/example");
        ...
        // http split with custom port
        httpsSplit = new Split("https://example.com:8443/example");
    ...
    {
        JsonCodec<Split> codec = jsonCodec(Split.class);
        String json = codec.toJson(split);
        Split copy = codec.fromJson(json);
